...
approximated by a linear shear in $x$, but left unaltered in
$z$. However, the vertical shear $\p_z\Omega$ is also assumed to only
depend on $z$.     
...
Before using the VGSB to study the VSI, it is very important to be
aware of several limitations of the box model. The VGSB momentum  
equation have the same form as that for the standard shearing box 
...
%horizontal structure not ported 
...
%equilibrium condition, which determines the vertical density/pressure
%structure. 
...
%but should be considered as a model parameter 
...
In Appendix we make the connection between the linear problem in the
VGSB to that in the global disk model.  
