...
\cite{barker15}. %Note, however, that \citeauthor{nelson13} and
% \citeauthor{barker15} employed the  approximations described in \S\ref{analytical} throughout their analysis, which are relaxed in our
% numerical study.  
...
The modes shown in Fig. \ref{compare_modes_iso_kx10} are inertial
waves made unstable by vertical shear and have been termed `body
modes' \citep{barker15}.  The `fundamental corrugation mode', marked by `f',
corresponds to the eigenvalue with the smallest $|\sigma|$. Hereafter
we will simply refer to this as the fundamental mode. The
fundamental mode is robust against vertical boundary conditions 
as it appears at the same position in
Fig. \ref{compare_modes_iso_kx10} for all cases. Higher  
order modes, corresponding to increasing $|\omega|$, increasingly
deviates from the analytic line due to the imposed boundaries, 
unless the vertical domain  size is increased \citep{barker15}. 
...
Fig. \ref{compare_modes_iso_kx10} lack `surface modes' found by 
\cite{nelson13}, \cite{mcnally14}  and by \cite{barker15}.  These are
a class of unstable modes associated with vertical shear near the disk
boundaries. 

We illustrate surface modes with the mode
...
of a surface mode in the fiducial disk. We typically find an increasing
number of surface modes with increasing $\zmax$ and/or $\khat$,
consistent with \cite{barker15}. 
 
As explained by \cite{barker15}, the occurence of surface modes 
occurence in the vertically isothermal disk is due to imposing a
finite vertical domain size. The fastest growing mode is a surface
mode, if they exist, but their association with vertical boundaries
means that $\mathrm{max}(\nu)$ depends on the choice of $\zmax$.   
We refer the reader to \citeauthor{barker15} for a
detailed analysis of surface modes. 
...
relaxation timescale $\beta$ for several values of $\khat$. 
We also plot the critical timescale $\beta_\mathrm{crit}=0.125$ for
the fiducial disk parameters.   
...
$\khat=5,\,10$ we find $\beta_\mathrm{crit}$ accurately predicts the
thermal timescal beyond which the VSI is suppressed.  

Complications from vertical boundaries arise when considering large
$\khat$. For $\khat=30$ we find for $\beta\gtrsim 0.1$ the dominant
disturbance correspond to the leftmost eigenvalues
seen in the lower two panels of Fig. \ref{compare_modes_cool_kx30},
which depend on $\zmax$. For $\khat=50,\,100$ we find surface modes
dominate.   
...
rates for $\beta\gtrsim 0.6$ are small. One might consider 
$\khat > 100$ to increase the maximum growth rate (note that the most unstable
mode for large $\khat$ are typically surface modes), but in a realistic disk
very small scale disturbances will likely be subject to viscous decay
(\S\ref{application}). Numerical simulations are also unlikely to
resolve such small scales.  

As pointed out by \cite{barker15} surface modes
only appear in vertically isothermal disks when vertical boundaries
are imposed, so they are strongly dependent on $\zmax$.  Thus, seeking
the most unstable mode in a vertically isothermal disk may not yield
robust results because such modes, which are often surface modes, may
depend on boundary conditions. 

\begin{figure}
   \includegraphics[width=\linewidth]{figures/gcorr_compare_iso_maxrate} 
...
Despite this caveat, and that surface modes are not accounted in our
analysis,  we see from Fig. \ref{bcrit_compare1} that 
$\beta_\mathrm{crit}$ nevertheless provides a reasonable estimate for
an upper limit to the thermal timescale for the VSI. We further 
test the dependence of $\beta_\mathrm{crit}$ on disk parameters
(Eq. \ref{iso_vsi_cond}).  We use the fiducial setup as a  reference
point and vary $q\in[-0.2,-1.2]$,  $\gamma\in[1.2,2.0]$, and
$\epsilon\in[0.02,0.1]$ separately. 

% We consider the fundamental
% corrugation mode for which $\beta_\mathrm{crit}$ is the actual critical
% thermal timescale. 
% We use $\khat=10$ to avoid complications to our
% eigenvalue search from surface modes.  

The numerically obtained $\beta_\mathrm{crit}$ are shown in
Fig. \ref{bcrit_compare} in comparison with Eq. \ref{iso_vsi_cond},
which generally agree. This result, together with 
Fig. \ref{bcrit_compare1}, shows that 
$\beta \lesssim \beta_\mathrm{crit}$ is a practical condition under  
which the VSI will operate effectively in stably stratified,
vertically isothermal disks.  

% The agreement improves with decreasing  $|q|$,
% $\epsilon$ and increasing $\gamma$, i.e. weaker instability. There is
...
non-isothermal 
disks have a disk surface at finite height so there is a natural
choice for $\zmax$. This means that higher order modes
and surface modes are better defined in a vertically non-isothermal
disk. However, note that this issue is unimportant for low order modes 
such as the fundamental mode, which are not sensitive to boundary
conditions.   
...
We do not find surface modes for $\khat=10$. In the vertically
...
most unstable mode without the boundary-dependent issue that would
arise in a vertically isothermal disk. 

\begin{figure}
  \includegraphics[width=\linewidth,clip=true,trim=0cm 0cm 0cm
  0cm]{figures/compare_modes_Gam1.084_kx100.ps}
  \caption{Same as Fig. \ref{compare_modes_vnoniso_kx10} but with
    $\khat=100$. In this case surface modes appear and are readily identified for
    $\beta=10^{-3}$ and nearly
...
$0.1$. At such large $\khat$, we find with $\beta=10^{-3}$ the most
...
Fig. \ref{bcrit_compare2} plots the maximum VSI growth rates in the
vertically non-isothermal disk model.  Based on
...
similar to the fiducial disk in  Fig. \ref{bcrit_compare1}. 
