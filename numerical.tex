...
isothermal since $c_s(\zmax) \simeq 0.9c_s(0)$. In Fig. \ref{omega_z}
...
modes' by \cite{barker15}.  The `fundamental corrugation mode', marked by `f',
corresponds to the eigenvalue with the smallest $|\sigma|$. Hereafter
we will simply refer to this as the fundamental mode, and it is robust
...
disk boundaries \citep{barker15}. They are also absent in our  
analysis because there we assumed a disk with infinite 
vertical extent, as the vertically isothermal disk has no
surface. However, surface modes can occur in the present calculations 
because there is an imposed vertical boundary.   

We illustrate surface modes with the mode diagram for $\khat=30$ in
...
imposing a finite vertical domain size. The fastest growing mode is a
surface mode, if they exist, but their association with vertical
boundaries means that $\mathrm{max}(\nu)$ depends on the choice of
$\zmax$. This has important implications when interpreting results
obtained from the vertically isothermal disk model. We return to this
issue later.    
...
which do not exist for $\zmax=3H$. For $\khat=50,\,100$ we find
surface modes dominate.   
...
\begin{figure}
  ...
  \includegraphics[width=\linewidth,clip=true,trim=0cm 1.75cm 0cm 0.9cm]{figures/gcorr_compare_iso_maxrate_z7} 
...
% However, note that this issue is unimportant for low order modes 
% such as the fundamental mode, which are not sensitive to boundary
% conditions. This is useful because when the fundamental mode is
% stabilized as $\beta$ is increased, higher order modes will have
% already been stabilized, the the growth rate of surface modes also
% significantly reduced. Then as far as a characteristic upper limit to
% the thermal timescale is conconcered, we may just consider the
% fundamental mode. 
