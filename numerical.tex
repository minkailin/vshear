...
In disks with weaker vertical stratification, i.e.\ $\Gamma > 1$, we expect $\beta_\mathrm{crit}$ to be larger.
...
We test $\beta_\mathrm{crit, noniso}$ using a disk model with 
...
behavior similar to Fig.\ \ref{bcrit_compare1} for the vertically
isothermal disk. For $\khat=O(10)$, growth rates drop to zero at the
predicted $\beta_\mathrm{crit,noniso}$.  As in the vertically isothermal case,
...
expect that a density weighted average of  $\beta_\mathrm{crit,noniso}$
