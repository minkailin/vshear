...
$c_s(\zmax)\simeq 0.94 c_s(0)$. Fig. \ref{omega_z} shows the
equilibrium rotation profile away from the disk mid-plane. 
...
{\bf ASK. Growth rates are expected to diminish for both $\khat\to 0$
...
developed in \S\ref{iso_poly}. For completeness we show the
...
similar to Fig. 3 in \cite{mcnally14}. The fundamental has the
smallest $|\sigma|$, for which the eigenvalue calculated from
Eq. \ref{sig2_iso} with $L=1$ and the numerically-obtained value are   
\begin{align*}
