...
setting $\beta=10^{-3}$. (Smaller values yield the similar results.) This
...
\begin{figure}
  ...
  \caption{Unstable modes with $\khat=10$ and thermal
    ...
    (bottom). Lines are computed analytically from
...
%fundamental mode bcomes the dominant mode as we approach beta
%critical, therefore can use beta crit as criteria 
...
the fundamental mode, which is again robust against boundary
conditions compared to higher order modes. We find surface modes
...
approaches $\beta_\mathrm{crit}$. %beta<beta_crit is necessary and sufficient  
