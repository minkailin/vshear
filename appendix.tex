...
% \end{align}
The function $F$ varies monotonically from $F=-1$ at $z=0$ to $F\to2$
...
t_c \gamma\right)$. Making the low frequency, nearly Keplerian
...
The VSI corresponds to unstable inertial waves. This is readily
obtained for large $\khat^2$ by balancing
the last two terms of Eq. \ref{simple_growth}  to obtain the low
frequency branch. Then 
\begin{align}
  \hat{\sigma}^2 \simeq L\left(\frac{1+\ii q \epsilon
       \hat{k}}{L+1+\hat{k}^2}\right).\label{simple_growth2}
\end{align}
This signifies instability for $L\geq1$ since we can choose the sign
