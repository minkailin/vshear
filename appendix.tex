...
suggests $\hat{\sigma}$ is complex for real $\khat$, which implies the
possibility of instability, regardless of the value of
$|\gamma-1|$. This is contradicts the second Solberg-Hoiland criterion 
(Eq. \ref{solberg2}), which imply instability requires 
$|\gamma-1|\ll1$ for thin, vertically isothermal disks evolved adiabatically. 
...
$\beta\ll1$, then the VGSB framework is an adequate approximation to
study the VSI.
...
\S\ref{iso_vsi_beta_crit}. For fixed $M$ of $O(1)$,  we assume
...
$\beta_\mathrm{crit}$ (Eq. \ref{iso_vsi_cond}) without the restrictive
assumption of $|\epsilon q \khat|\ll1$, as used in the main text to obtain the
equivalent expression for $M>0$ (Eq. \ref{bcrit_gen}).

%  In this case the fourth term
% in Eq. \ref{relax_cond1} may be retained but other aforementioned terms
% are still neglected. One then obtains an expression for $\beta$ by
% eliminating $\hat{\omega}\khat$ between Eq. \ref{relax_cond1} ---
% \ref{relax_cond2}. The expression for the critical thermal timescale
% for the fundamental mode, $\beta_\mathrm{crit}$
% (Eq. \ref{iso_vsi_con}, is then obtained by taking the thin disk
% limit. 
...
\end{align}
This is just the dispersion relation for axisymmetric isothermal waves in a
...
A simple solution for $\hat{\sigma}$ may be obtained by balancing
the  last two terms of Eq. \ref{simple_growth} to obtain the
low-frequency branch,  
\begin{align}
  \hat{\sigma}^2 \simeq L\left(\frac{1+\ii q \epsilon
       \hat{k}}{L+1+\hat{k}^2}\right).\label{simple_growth2}
\end{align}
This signifies instability for $L\geq1$ 
