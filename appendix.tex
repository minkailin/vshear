...
\section{Eigenfrequency in locally
  isothermal disks in global cylindrical geometry}
Consider a locally isothermal disk in 3D such that $P=c_s^2\rho$ at
all times, so in the perturbed state $\delta P =
c_s^2\delta\rho$, with $c_s(r)$ begin a prescribed function. 
The linearized fluid equations in cylindrical
geometry is then 
\begin{align}
...
\end{align}
We can eliminate $\delta v_\phi$ from these equations and obtain
\begin{align}
...
$\nabla\cdot\left(\rho\delta\bm{v}\right)$ is negligible in this equation \citep{nelson13},
then the only contribution to instability comes from the third term on
the left hand side, i.e. the explicit vertical shear term $\p_zj$. The 
term proportional to $\kappa^2$ is stabilizing. Because radial shear
...
If we integrate Eq. \ref{global_integral} over a volume
...
\begin{align}
  ...
  \bm{v}^*\cdot\nabla c_s^2.
\end{align}
Recalling $j=r^2\Omega$ and $r\p_z\Omega^2 = -\p_z\ln{\rho}\p_r c_s^2$
for a locally isothermal disk, we obtain
\begin{align}
  \sigma^2\int \rho\left(|\delta v_r|^2 + |\delta v_z|^2\right) dV =
  \int \kappa^2\rho|\delta v_r|^2 dV + \int
  \frac{c_s^2}{\rho}\left|\nabla\cdot\left(\rho\delta\bm{v}\right)\right|^2
  dV + \int \left[ \rho \left(\nabla\cdot\delta\bm{v}\right)\delta
    v_r^* + |\delta v_r|^2\frac{\p \rho}{\p r}\right]\frac{dc_s^2}{dr}  dV.
\end{align}
