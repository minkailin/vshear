...
% \end{align}
% Since we have assumed $|\sigma^2|\ll \kappa^2\sim \OmK^2$, our
...
To investigate marginal stability we set $\hat{\sigma} = 0$ so the frequency, $\hat{\upsilon}=\hat{\omega}_c$, is real.  
We take the short radial wavelength limit, $\khat^2\gg 1$, of the coefficients in Eq.\ \ref{coeffs}.  The real and 
imaginary parts of Eq. \ref{relax_disp} then give relations for $\hat{\omega}_c$ and $\beta_c$, the cooling time for 
marginal stability, as 
\begin{align}\
...
$\gamma>1$ but is $O(1)$. We consider $M$ to be $O(1)$ or smaller, so 
that $|\hat{\omega_c}\khat|=O(1)$, since for inertial waves
$|\hat{\omega}\khat|^2\simeq 1 + M $. Finally, we assume 
$\beta_c \ll 1$, to be justified \emph{a posteriori}. Applying these
approximations give
Eq. \ref{relax_cond_simp1}---\ref{relax_cond_simp2}.   

\subsection{Marginal stability for the fundamental mode}\label{bcrit_alt}
If we apply the above approximations for the fundamental mode $M=0$ to
Eq. \ref{relax_cond1}---\ref{relax_cond2}, but with the less
restrictive assumption $\beta_c \lesssim O(1)$, we expect the balance  
...
\end{align}
which reduces to Eq. \ref{iso_vsi_cond} for thin-disks ($h\ll1$). We
thus deduce that in fact $\beta_c\ll 1$, which motivates us to make this
strong assumption from the onset.   
