...
\end{align}
We next (re)-apply the low frequency and thin approximations, $h, \hat{\omega}_c \ll 1$ and 
further apply the rapid cooling limit $\beta_c \ll 1$.  We retain terms with the combinations 
$\hat{\omega}_c \hat{k}$, $\beta_c/h$, and $h \hat{k}$ which could be order unity.  For inertial waves we know that
$\hat{\omega}_c \sim 1/\hat{k}$ holds, while $\beta_c/h \sim O(1)$ will be justified \emph{a posteriori}
and the magnitude of $h \hat{k}$ depends on the choice of wavenumber.  Applying this ordering we 
arrive at Eq. \ref{relax_cond_simp1}---\ref{relax_cond_simp2}.
