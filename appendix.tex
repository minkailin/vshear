...
\begin{align}
  & \ii\zeta\delta v_z = \frac{dW}{dz} + \frac{d\ln{\rho}}{d z}\left(W-Q\right),\label{ode_vz} \\
  &\frac{\zeta^2}{c_s^2}Q + \frac{\zeta^2k_x^2}{D}W =\ii\zeta \left(\frac{\ii
      k_x r}{D}\frac{d\Omega^2}{dz} -
    \frac{d\ln{\rho}}{dz}\right)\delta v_z - \ii\zeta \frac{d\delta v_z}{dz},
...
%    1\right)\delta v_z,  
  &\zeta^2W - \gamma\zeta^2 Q +
  \frac{\ii\zeta}{t_c}\left(W-Q\right) 
  =\ii \zeta c_s^2\frac{d\ln{\rho}}{dz}\left(\gamma -
    1\right)\delta v_z,
  % 
  % \left[\frac{dW}{dz} + \frac{d\ln{\rho}}{dz}\left(W-Q\right)\right],
  \label{ode_Q} 
...
\begin{align}\label{full_ode}
  ...
  &+ \left\{\zeta^2\left(\frac{k_x^2}{D} +
   ...
   \right\}\delta v_z,
\end{align}
where we recall $\chi = \left(1-\ii\zeta t_c\right)/\left(1-\ii\zeta
...
\begin{align}
  ...
  -\hat{\zeta}^2\left(\hat{k}^2+\chi\right)\delta v_z\label{adia_diso3}.
\end{align} 
...
\begin{align}
  ...
  +\left\{\hat{\zeta}^2\left(\frac{1}{\gamma}+\hat{k}^2\right) 
...
% $\hat{\zeta}^2$ is real, and  $\hat{\zeta}^2>0$ if $\gamma>1$. As
...
suggests $\hat{\zeta}^2$ is complex for real $\khat$, which implies the
...
\begin{align} 
  Q = W 
\end{align}
(i.e. $\delta P = c_s^2\delta \rho$)% , and so $  \ii\hat{\zeta}c_s\delta v_z = W^\prime $
% from Eq. \ref{lin_vz}
...
\begin{align}
  0 = W^{\prime\prime} + \left[\ln\rho^\prime - \frac{\ii h q \hat{k} f(\zhat)
      }{1-\hat{\zeta}^2} \right]W^\prime +
  \hat{\zeta}^2\left(1+\frac{\hat{k}^2}{1-\hat{\zeta}^2}\right)W,\label{iso_ode0} 
\end{align}
...
In the low-frequency limit we assume $|\hat{\zeta}|\ll 1$ and
approximate Eq. \ref{iso_ode0} as 
\begin{align}
  0 = W^{\prime\prime} + \left[\ln\rho^\prime - \ii h q \hat{k}
    f(\zhat)\right]W^\prime +
  \hat{\zeta}^2\left(1+\hat{k}^2\right)W. \label{iso_ode1}
\end{align}
...
\begin{align}
  \hat{\zeta}^2\left(1+\hat{k}^2\right)\int_{\zhat_1}^{\zhat_2}\rho|W|^2d\zhat
...
\end{align}
It follows that for instability ($\imag\hat{\zeta}>0$), it is necessary to
...
shear rate in the domain. % Writing $\hat{\zeta} = \hat{\omega} +
% \ii\hat{\zeta}$ with real $\hat{\omega}$ and $\hat{\zeta}$, 
...
\end{align}
where we recall $\hat{\omega} = \real \hat{\zeta}$ and
$\hat{\sigma}=\imag\hat{\zeta}$. 
Adding the square of these equations give
\begin{align}
  \left[|\hat{\zeta}|^2\left(1+\hat{k}^2\right)
    \int_{\zhat_1}^{\zhat_2}\rho|W|^2d\hat{z} -
...
\begin{align}\label{iso_ode3}
  W^{\prime\prime} - \left(1 + \frac{\ii qh \hat{k}}{1-\hat{\zeta}^2}\right)\hat{z}W^\prime  +
  \hat{\zeta}^2\left(1+\frac{\hat{k}^2}{1-\hat{\zeta}^2}\right)W = 0.
\end{align}
...
\begin{align}
  (l+2)(l+1)a_{l+2} +
  \left[\hat{\zeta}^2\left(1+\frac{\khat^2}{1-\hat{\zeta}^2}\right)
    - l\left(1+\frac{\ii h q
        \hat{k}}{1-\hat{\zeta}^2}\right)\right] a_l = 0. 
\end{align}
...
$|\zhat|\to\infty$.  Then the eigenfrequency $\hat{\zeta}$ is given via 
\begin{align}
\hat{\zeta}^4 - \left(L+1+\khat^2\right)\hat{\zeta}^2 + L\left(1 +
...
written as \begin{align}
  \hat{\zeta}^2\khat^2 =
  \left(L-\hat{\zeta}^2\right)\left(1-\hat{\zeta}^2\right), \label{iso_disp_full}
\end{align}
...
\He_L(\zhat)$.  The eigenfrequency $\hat{\zeta} = \hat{\omega}$ is real and the disk
...
\begin{align}
  \hat{\zeta}^2 \simeq L\left(\frac{1+\ii q h
...
of the square root to make $\hat{\sigma} = \imag\hat{\zeta}>0$.  These
...
%   \hat{\zeta}^2 = L\left(\frac{1+\ii q h
%       \hat{k}}{1+\hat{k}^2}\right).
% \end{align}
% The eigenfrequency $\hat{\zeta}$ is therefore complex for
...
% unstable modes for which $\hat{\zeta}>0$ and is given by
% \begin{align}
%   \hat{\zeta} =\sqrt{
...
% and
% \begin{align}
%   \mathrm{max}\left(\hat{\zeta}\right) =\frac{\sqrt{L}|h
...
Assuming neutral modes with real $\hat{\zeta}=\hat{\omega}$ and $\khat^2\gg 1$, the real and imaginary parts of
...
\begin{align}
%  \rho_0 = 2.7\times10^{-9}
%  \hat{\zeta}\left(\frac{r}{\mathrm{AU}}\right)^{-39/14}\mathrm{g}\,\mathrm{cm}^{-3},  
\rho_0 = 1.7\times10^{-9}
