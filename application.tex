...
\end{align}
\citep[see also][]{barker15} where $\eta=k_\mathrm{rad}/\rho c_v$ is
...
Our analytical criteria is not valid for non-uniform $\beta$, and we must
...
of $\khat$ such that $\beta = \beta_\mathrm{crit}$, which show that in
the inner disk, say $r\sim 1$AU, the VSI would only occur on very
small radial lengthscales. 
...
wavenumbers $\khat\gtrsim 50$. However, surface modes are dependent on
boundary conditions in a vertically isothermal disk \citep{barker15},  
and disturbances with large $\khat$ are more likely subject to viscous
decay, as discussed above. % It is also unclear if surface modes are
