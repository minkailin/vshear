...
suggest that significant portions of PPDs fail to develop MHD turbulence \citep[e.g.][]{simon13, lesur14,bai15,gressel15}.

A purely hydrodynamic mechanism could circumvent difficulties with non-ideal MHD, but must overcome
the strong centrifugal stability imposed the positive radial angular momentum gradient in nearly Keplerian disks \citep{balbus96}.
One possible route to hydrodynamic turbulence is the vertical shear
instability (VSI, \citealp{urpin98, nelson13}, hereafter N13).  The basic mechanism of the VSI  in disks was first
...
the stabilizing effects of vertical buoyancy. In principle, vertical buoyancy can be eliminated by strong internal heating, i.e.\ by the onset of convection.  
Real PPDs should be vertically stably stratified in the outer regions, beyond $\sim 1$--5 AU, where heating is 
irradiation dominated \citep{bitsch15}.  In the inner disk, strong vertical buoyancy is possible if accretion heating is weak
or is concentrated in surface layers \citep{gammie96,lesniak11}.

To overcome vertical buoyancy, the VSI requires a short cooling time (\citealp{goldreich67}; N13).  Rapid radiative cooling 
brings a moving fluid into thermal equilibrium with its surroundings, thereby diminishing buoyancy.  An isothermal equation
of state implies instantaneous cooling, and is the ideal context for studying the VSI (\citealp{urpin03}, N13, \citealp{barker15}). 

A full understanding of the VSI in real disks requires considering finite cooling times, $t_{\rm c}$.
Simulations with a fixed, finite  $t_{\rm c}$ find that  VSI turbulence in stably stratified disks requires rapid cooling 
with $t_{\rm c} \lesssim 0.01/\Omega$, in terms of the orbital frequency $\Omega$ (N13).  N13 further find that the VSI can drive moderately strong transport, with Reynolds stresses of $\alpha \sim 10^{-3}$ times the mean pressure.    Simulations with realistic radiative transfer, in lieu of a constant $t_{\rm c}$ approximation,  find that the VSI in irradiated disks drive transport with $\alpha \sim 10^{-4}$
 in a  $\sim 2$ -- 10 AU PPD model \citep{stoll14}.

This paper presents the first comprehensive study of how finite cooling times affect the linear development of the VSI.  We consider models with constant $t_{\rm c}$  and  also with more realistic cooling from a dust opacity in both the optically thick (diffusive) and thin regimes, where appropriate.  Coupled with the results from numerical simulations, our results can be used to understand how the VSI operates over a wide range of model PPDs.  This paper is organized as follows. 
In \S\ref{vsi_require} we introduce and qualitatively explain our main result, the critical cooling time for the VSI.
