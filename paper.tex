...
\begin{align}
  t _c \ll \Omega^{-1},  
\end{align}
consistent with numerical experiments \citep{nelson13}. For this
preliminary discussion we shall consider a vertically isothermal disk
with aspect-ratio $\epsilon$, 
...
\begin{align}\label{vshear_thin}
  r \frac{\p \Omega^2 }{\p z} \simeq \epsilon q \Omega^2\left(\frac{z}{H}\right),
\end{align}
...
that temperature flucations decay rapidly (i.e. nearly isothermal
perturbations), then the fluid element would attain its surrounding
density and  
...
How short must $t_c$ be for vertical shear to prevail? For a rough
comparison of vertical shear to buoyancy for non-adiabatic
perturbations, we apply Eq. 47 of \cite{urpin03}, which is based on
\cite{townsend58},   
...
which is destabilizing. In a thin disk, the destabilizing effect of
vertical shear is rather weak because $|r\p_z\Omega|\propto \epsilon
\ll 1$. By comparison the buoyancy frequency $N_z/\Omega\sim 1$ at
$z\sim H$.  
...
frequencies are normalized by the local Keplerian frequency. It is
clear that vertical shear is 
generally weak compared to buoyancy. Then even a small (positive)
vertical entropy gradient can significantly stabilize the VSI. It
follows that $t_c$ must be quite short to permit instability.  
...
with height, as evident in
Fig. \ref{eqm_structure}. Eq. \ref{bcrit_est} then suggest that with
...
%it would imply you can always have VSI sufficiently close to the
%midplane 
...
reflects Eq. \ref{bcrit_est} but is simpler since there is no
$z$ dependence. \emph{In a thin 
...
satisfies Eq. \ref{prelim_bcrit}. The criterion Eq. \ref{prelim_bcrit} in fact
