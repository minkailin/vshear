...
\begin{abstract}
  ...
  astrophysical disks. % with, for example, radial variations in temperature. 
  ...
  we find the VSI would only operate on extremely short lengthscales $\ll 
  O(10^{-2}H)$, where $H$ is the disk scale height. On longer
  lengthscales $O(10^{-2}H)$ to $O(10^{-1}H)$, we find the MMSN is
...
stratified and have non-zero thermal timescales. For example, we show in  
Fig. \ref{bcrit_mmsn2d} an estimate of the thermal timescales in a
typical protoplanetary disk model (see below). In order to determine
...
\begin{figure}
  ...
  0cm]{figures/bcrit_mmsn2d} 
  \caption{Estimate of the thermal timescale  in a fiducial
    protoplanetary disk model, the Minimum Mass Solar Nebula, for
    perturbations with radial lengthscales $\sim 10^{-2}H$, where $H$
    is the disk scale-height. The red line corresponds to a 
    characteristic thermal timescale, derived in this work, 
    below which the VSI is expected to operate effectively. 
    \label{bcrit_mmsn2d} 
  }
\end{figure}
...
timescale for the VSI. In \S\ref{numerical} we numerically
