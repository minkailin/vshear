...
\begin{abstract}
  ...
  timescale in stably stratified disks. %which is usually assumed 
  ...
  we study the linear, axisymmetric VSI in radially local but
  vertically global models of accretion disks in which the disk  
  temperature $T$ is restored to its initial value over a characteristic timescale 
  ...
  varying thermal timescale $\beta$. We find increasing $\beta$ even slightly away from 
  zero can dramatically reduce VSI growth rates. % , since this generally 
  % introduces a vertical entropy gradient that stabilizes vertical
  % motions as needed for the VSI.
  ...
  $\gamma$ is the adiabatic index. Our  linear calculations are consistent with previous hydrodynamic 
  simulations. We apply our linear calculations  
  ...
  periods. We find the dominant VSI mode in the MMSN is the `corrugation' mode
  which casues the disk column to oscillate vertically. 
\end{abstract}
...
$\Omega$ depends on the height $z$ from the disk midplane. This dependence
occurs when the equilibrium pressure depends on both the temperature
and density, making the disk baroclinic. It is thus generally
expected that $\p_z\Omega\neq 0$ in astrophysical disks. 
...
\cite{nelson13} and \cite{stoll14}. In these simulations the authors
...
stratified. Since the latter is usually
assumed for typical disk models,  
% disk thermodynamics except
% in the special case of neutrally stratified disks. 
it is 
...
the VSI in realistic disks. % Numerical simulations of the VSI are
% challenging, however, because global disk simulations should be
% carried out while resolving radial scales much smaller than the local
% disk scale height.    
%On the other hand, 
Should a given disk become turbulent through the VSI, it must pass through the
linear instability first. Thus, understanding the thermodynamic dependence
of the linear VSI is essential. % Furthermore, linear
% stability calculations are essential to complement future numerical simulations.  
...
isothermal perturbations. Our models are also an extension of earlier 
