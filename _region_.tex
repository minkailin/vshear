\message{ !name(linear_iso.tex)}
\message{ !name(linear_iso.tex) !offset(-2) }
...
We can use the explicit solution developed above to examine the effect
of cooling $t_c$ on perturbations with $\gamma>1$. We shall consider
the limit $t_c\to0$, for which perturbations can be considered
nearly-isothermal.  
...
Since we have developed solutions for $\beta\equiv 0$, we may ask how
do the eigenfrequencies and eigenfunctions change when we change
$\beta$ from zero to a small but finite value? For sufficiently small
$\beta$, we expect the new solutions to only differ slighly from that
above. Then we may linearize Eq. \ref{nearly_iso1}---\ref{nearly_iso2} to
...
and $\hat{\sigma}^2$ is given by Eq. \ref{sig2_iso}. Inserting
...
We could proceed further by considering the set of polynomial
solutions for $W$ described in \S\ref{iso_poly} altogether, but it is
simplest to look at the fundamental mode. Thus
we consider the `basic state'
\begin{align*}
...
stabilization by finite cooling is ineffective at very large or very small scales. 


\begin{figure}
  \includegraphics[width=\linewidth]{figures/domegadbeta}
  \caption{Rate of change of the eigenfrequency $\sigma$ with respect to the
    introduction of a small but finite cooling time. The plotted 
    quantity is $\p\hat{\sigma}/\p\beta$ at $\beta=0$, evaluated using
    Eq. \ref{nearly_iso_dsig} with 
    disk parameters $\epsilon=0.05$, $q=-1$ and $\gamma=1.4$.  
    \label{domegadbeta}}  
\end{figure}   
\message{ !name(linear_iso.tex) !offset(-393) }
