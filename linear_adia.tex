...
$D\to \Omega_k^2$ in Eq. \ref{adia_iso1}---\ref{adia_iso2}. For the
analysis to be carried out, the latter is a more limiting
approximation than for isothermal perturbations because we make the
replacement before combining the pair of equations above. 



Here, we also use the fact that in the
global disk, 
\begin{align}
...
Making the above substitutions and combining Eq. \ref{adia_iso1}---
...
$\hat{k}\gg1$. Thus, instability is expected to associated with the 
...
vanish. Furthermore, we require 
the vertical kinetic energy density $\rho|\delta v_z|^2$ to remain
finite. Then assuming $g(\zhat)$ is a polynomial, we require 
\begin{align}
  \real\alpha < \frac{1}{2}. 
\end{align}
We therefore choose 
...
\begin{align}
  ...
  % q|}{2\times3^{3/4}}\notag.
\end{align}
Unlike for isothermal perturbations, Eq. \ref{gam2_max_growth} show
that growth rates decrease with $M$, and there exists a maximum
possible growth rate at $M=0$. This is despite the fact that in the
thin-disk approximation, for which $d\Omega^2/dz\propto z$, there is
infinite vertical shear available.  

The acceptable solution for $\alpha$ is
\begin{align}
...
eventually decays for large heights. This is unlike for isothermal
perturbations, in which perturbation magnitudes increase with $|z|$.  
