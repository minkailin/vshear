...
entropy gradient, or buoyancy frequency $N_z$. 
...
for $N_z$ appropriate for vertically isothermal disks, we expect
\begin{align}
  \beta \ll
  \frac{1}{H_\mathrm{iso}\sqrt{\frac{\gamma-1}{\gamma}}\left|\frac{\p\ln\rho}{\p
          z}\right|} 
\end{align}
is required to enable the VSI. This inequality is satisfied at the
midplane and as $|z|\to\infty$. It can be satisifed at all heights if
it is satisfied at the height of $\mathrm{max}|\p\ln\rho/\p z|$. 
Using the expression for the density field for vertically isothermal
disks (Eq. \ref{vertiso_eqm}), we expect 
\begin{align}
  \beta \ll \epsilon\frac{3^{3/2}}{2}\sqrt{\frac{\gamma}{\gamma-1}},
\end{align}
i.e. $\beta\ll O(\epsilon)$, is a sufficient condition for
