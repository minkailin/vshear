...
that $\chi = 1$. This case has been investigated numerically
by \cite{nelson13} and \cite{mcnally14}. 

In Appendix \ref{iso_discuss}, we give an analytical discussion of the
linear problem for isothermal perturbations. There, we establish more general
...
the maximum vertical shear rate in the disk). We also develop 
solutions in the thin disk limit. The 
methods used in Appendix \ref{iso_discuss} and results obtained
...
solve for $\delta\hat{\sigma}$ using the above xpressions for $\delta\chi$
...
\end{align}
Since the second term in square brackets is small by assumption, we have 
...
in fact be obtained without assuming $|\epsilon q\khat|\lesssim 1$.  We will confirm Eq. \ref{iso_vsi_cond} numerically in 
