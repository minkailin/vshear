...
equilibrium density and pressure field in the VGSB is homogeneous in
the radial ($x$) direction, as in the standard shearing box.   

Thus, the linear problem in the VGSB can be derived from
Eq. \ref{lin_mass}---\ref{lin_energy} by setting 
$\hat{g}_c=0$. The resulting linear problem is very similar 
...
%$\beta=0$. 
($\chi=1$) and Eq. \ref{ode_Q} becomes  
\begin{align}
  Q = W 
\end{align}
(i.e. $\delta P = c_s^2\delta \rho$), and so  
\begin{align}
...
\end{align}
For a vertically isothermal disk we have 
\begin{align}
  f(\zhat) = \frac{\zhat}{\left(1+\epsilon^2\hat{z}^2\right)^{3/2}}, 
\end{align}
%For the vertically isothermal disk we have $|f(\hat{z})| =
%|\hat{z}|\left(1+\epsilon^2\hat{z}^2\right)^{-3/2}$, 
which maximizes at $|\zhat|=1/\sqrt{2}\epsilon$. The existence of a
maximum vertical shear limits the growth rate of unstable modes, as  
shown below.   

Note that Eq. \ref{iso_ode1} can in fact be derived by substituting
$Q=W$ in Eq. \ref{ode_w} without approximating $D$ or $\kappa^2$ as
vertically constant.    

% the issue of ignoring the
% vertical dependence of $D$ described in \S\ref{approx_gov} and Appendix
% \ref{adia_improve} is irrelevant, since
% Eq. \ref{iso_ode1} may be derived directly , i.e. no derivatives of $D$ needs to be taken.
...
unstable modes for which $\hat{\nu}=\imag{\hat{\sigma}}>0$ is the
growth rate and is given by 
